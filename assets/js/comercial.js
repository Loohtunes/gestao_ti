...
const ETAPAS_CONFIG = {
  ...
  medicao: {
    id: 'medicao', nome: 'Medição', short: 'Medição', opcional: true, cor: '#22c55e',
    subEtapas: [
...
function setFiltroRep(val) { _filtroRep = val; renderObras(); }
function setFiltroEtapa(val) { _filtroEtapa = val; renderObras(); }
...
function getEtapaAtualId(obra) {
  ...
  return obra.concluida ? 'concluida' : 'proposta';
}
...
function renderObras() {
  ...
  if (counter) counter.textContent = `${lista.length} obra${lista.length !== 1 ? 's' : ''}`;
  if (!lista.length) {
    el.innerHTML = `<div class="obras-empty">
      ...
      <h3>${_filtroRep || _filtroEtapa ? 'Nenhuma obra com esses filtros' : 'Nenhuma obra cadastrada'}</h3>
      <span style="font-size:0.78rem;">Clique em "+ Nova Obra" para começar</span>
    </div>`;
    return;
  }
  el.innerHTML = lista.map(obra => {
    const etapaId = getEtapaAtualId(obra);
    const cfg = ETAPAS_CONFIG[etapaId];
    const cor = cfg?.cor || 'var(--muted)';
    const etapaNome = obra.concluida ? 'Concluída' : (cfg?.short || '—');
    const badgeCls = obra.concluida ? 'etapa-concluida' : `etapa-${etapaId}`;
  ...
  tl.innerHTML = ETAPAS_ORDER.map(etapaId => {
    ...
    const subsHtml = cfg.subEtapas.map(subCfg => {
      ...
      if (subData.dataConclusao) {
        dataInfo = `<span class="sub-data concluida">✓ ${new Date(subData.dataConclusao + 'T12:00:00').toLocaleDateString('pt-BR')}</span>`;
      } else if (limite) {
...
async function escolherProximaEtapa(opcao) {
  ...
  if (opcao === 'concluir') {
    ...
    // Ativa Aditivos agora; Medição será ativada ao concluir Aditivos
    etapas['aditivos'].ativa = true; etapas['aditivos'].status = 'active';
    etapas['medicao'].ativa = true;  // marca para ativar depois
