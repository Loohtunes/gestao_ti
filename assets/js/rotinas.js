...
function calcTotais() {
  ...
  if (el) el.innerHTML=`
    ...
    <div class="imp-total-item total"><span>📄 Total Geral</span><strong>${(color+mono).toLocaleString('pt-BR')} pgs</strong></div>`;
}
...
async function saveImpressorasRegisto() {
  ...
  try {
    ...
    const leituras = _impressoras.map(imp=>{
      ...
      return { impId:imp.id, nome:`${imp.marca} ${imp.modelo}`, tipo:imp.tipo,
    ...
    await db.collection('rotinas_impressoras').add({
      ...
      registradoEm:firebase.firestore.FieldValue.serverTimestamp(),
      registradoPor:currentUser.username, leituras,
      faturamento:isA?{franquiaPags,franquiaValor:franquiaVal,valorPorPag:valorPag,
...
async function saveCFTVRegisto() {
  ...
  try {
    const dvrs=_dvrs.map(dvr=>{
      ...
      return {dvrId:dvr.id, nome:dvr.nome, status:_dvrStatus[dvr.id]||'ATIVO',
    ...
    _cameras.forEach(cam=>setCFTVStatus(cam.id,cam.dvrId,'ATIVO'));
...
async function loadUltimosRegs() {
  ...
  if(!c.empty){const d=c.docs[0].data();_ultimosReg.cftv=d.data;_ultimosReg.cftv_por=d.registradoPor;}
...
function renderHistoricoTable() {
  ...
  el.innerHTML = _historico.map(r => {
    ...
    return `<tr class="hist-main-row" onclick="openHistModal('${r._id}')" title="Clique para ver detalhes">
      <td>${badge}</td>
      <td style="font-family:var(--font-mono);font-size:0.78rem;">${dataStr}</td>
